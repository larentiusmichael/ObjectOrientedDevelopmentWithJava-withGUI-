...
public class PersonnelAccount extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PersonnelAccount frame = new PersonnelAccount(fullName, username, id);
	 ...
	 * Create the frame.
	 */
	public PersonnelAccount(String fullName, String username, String id) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				dateTime.setAction("VIEWING ACCOUNT");
				dateTime.setUsername(username);
				dateTime.setID(id);
				dateTime.activityRecord();
				new PersonnelAccountView(fullName, username, id).setVisible(true);
				PersonnelAccount.this.dispose();
			}
		});
		
		JButton btnNewButton_1 = new JButton("Modify Account Details");
